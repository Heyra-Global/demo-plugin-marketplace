...
if printf '%s' "$CMD" | grep -Eq 'git[[:space:]]+push' \
  ...
  emit deny "heyra-dev guard: force-push to a protected branch is not allowed from a Claude session. Open a pull request instead."
fi

# 2. Destructive local operations: ask.
if printf '%s' "$CMD" | grep -Eq '(^|[[:space:]|;&])rm[[:space:]]+-[a-zA-Z]*r'; then
  emit ask "heyra-dev guard: recursive delete. Confirm the path before it runs."
fi
if printf '%s' "$CMD" | grep -Eq 'git[[:space:]]+(reset[[:space:]]+--hard|clean[[:space:]]+-[a-zA-Z]*f|checkout[[:space:]]+--[[:space:]]+\.|branch[[:space:]]+-D)'; then
  emit ask "heyra-dev guard: this git command discards work. Confirm before it runs."
fi
